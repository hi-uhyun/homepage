...
type Category = 'voice-acting' | 'acting' | 'singing';

const CATEGORIES: Category[] = ['voice-acting', 'acting', 'singing'];
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border border-zinc-100 bg-white shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="p-4">
        ...
        {item.tone && (
          <span className="mt-2 inline-block rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs text-zinc-500">
            {item.tone}
          </span>
        )}
...
export default function PortfolioPreview({ items, locale }: PortfolioPreviewProps) {
  ...
  const categoryLabels: Record<Category, string> = {
    ...
    'acting': t('portfolio.acting'),
    'singing': t('portfolio.singing'),
  };
  ...
  const featuredByCategory = CATEGORIES.reduce<Record<Category, PortfolioItem[]>>(
    ...
    { 'voice-acting': [], acting: [], singing: [] }
  );

  return (
    <section
      className="bg-zinc-50 px-4 py-20"
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-12 flex items-end justify-between">
          ...
          <Link
            ...
            aria-label={locale === 'ko' ? '포트폴리오 전체 보기' : 'View full portfolio'}
          >
            {t('common.viewAll')}
          </Link>
        </div>
